...
(() => {
  ...
  function Pages({ data, answers=false, offset=0, total=1 }) {
    ...
    return pages.map((items,pageIndex)=><article className="worksheet-page generic-page" key={`${answers}-${pageIndex}`}>{data.header?<div className="worksheet-header-card mb-4 rounded-xl border border-black p-4"><div className="flex items-center gap-3"><img src={DEF.icon} className="h-12 w-12 object-contain" alt=""/><div className="min-w-0"><h1 className="worksheet-header-title text-xl font-bold leading-tight">{answers ? `${data.title} - Answers` : data.title}</h1>{!answers&&<p className="worksheet-header-instructions text-sm text-stone-700">{data.instructions}</p>}</div></div>{!answers&&pageIndex===0?<div className="mt-4 flex gap-5 text-sm">{data.name&&<span className="flex min-w-0 flex-[2] items-end gap-2"><span className="shrink-0">Name:</span><span className="pupil-detail-line mb-[3px] h-px min-w-8 flex-1 bg-black"/></span>}{data.classField&&<span className="flex min-w-0 flex-1 items-end gap-2"><span className="shrink-0">Class:</span><span className="pupil-detail-line mb-[3px] h-px min-w-8 flex-1 bg-black"/></span>}{data.date&&<span className="flex min-w-0 flex-1 items-end gap-2"><span className="shrink-0">Date:</span><span className="pupil-detail-line mb-[3px] h-px min-w-8 flex-1 bg-black"/></span>}</div>:null}</div>:null}<div className={`generic-grid grid flex-1 ${CONFIG.activityId==="barlines"||CONFIG.activityId==="timesig"||CONFIG.activityId==="triplets"||CONFIG.activityId==="rests"?"grid-cols-1":"grid-cols-2"} ${data.gridlines?"with-gridlines":""}`}>{items.map((q,i)=>{const example=!answers&&pageIndex===0&&i<examples.length; const n=answers?pageIndex*perPage+i+1:rhythmSums?(pageIndex===0?i-examples.length+1:perPage+(pageIndex-1)*perPage+i+1):pageIndex*perPage+i-examples.length+1; return <Question key={q.id} question={q} number={n} example={example} answers={answers} numbers={data.numbers} marks={data.marks} firstPage={!answers&&pageIndex===0} continuationPage={pageIndex>0}/>})}</div>{!answers&&data.marks&&pageIndex===pages.length-1?<div className="mt-3 flex justify-end gap-3 text-sm font-bold"><span className="h-8 leading-8">Total marks</span><span className="total-marks-box relative h-8 w-24 border border-black"><span className="total-marks-value absolute left-2 right-2 text-right leading-none">/ {data.questions.length}</span></span></div>:null}<footer className="mt-2 flex justify-between text-[10px] text-stone-400"><span>The Music Literacy Hub</span><span>Page {offset+pageIndex+1} of {total}</span></footer></article>);
  }
  ...
  function PracticeWorksheetApp() {
    ...
    const data={title,instructions,header,name,classField,date,gridlines,numbers:true,marks:true,paperCount:count};
  ...
  function GenericApp() {
    ...
    const data={questions,title,instructions,header,name,classField,date,numbers,marks,gridlines,examples:activeExamples,example:activeExamples[0]||null};
    ...
    const check=(label,value,setter,disabled=false)=><label className={`flex items-center gap-2 whitespace-nowrap text-sm ${disabled?"text-stone-400 opacity-60":""}`}><input className="worksheet-checkbox" type="checkbox" checked={value} onChange={e=>setter(e.target.checked)} disabled={disabled}/>{label}</label>;
    const changeHeader=(enabled)=>{setHeader(enabled);if(!enabled){setName(false);setClassField(false);setDate(false);}};
    async function outputPdf(mode){
      if(preparingMode)return;
      setPreparingMode(mode);
