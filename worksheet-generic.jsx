...
(() => {
  ...
  function Pages({ data, answers=false, offset=0, total=1 }) {
    ...
    return pages.map((items,pageIndex)=><article className="worksheet-page generic-page" key={`${answers}-${pageIndex}`}><div className="worksheet-header-card mb-4 rounded-xl border border-black p-4"><div className="flex items-center gap-3"><img src={DEF.icon} className="h-12 w-12 object-contain" alt=""/><div className="min-w-0"><h1 className="text-xl font-bold leading-tight">{answers ? `${data.title} - Answers` : data.title}</h1>{!answers&&<p className="text-sm text-stone-700">{data.instructions}</p>}</div></div>{!answers&&pageIndex===0?<div className="mt-4 flex gap-5 text-sm">{data.name&&<span className="flex min-w-0 flex-[2] items-end gap-2"><span className="shrink-0">Name:</span><span className="pupil-detail-line mb-[3px] h-px min-w-8 flex-1 bg-black"/></span>}{data.classField&&<span className="flex min-w-0 flex-1 items-end gap-2"><span className="shrink-0">Class:</span><span className="pupil-detail-line mb-[3px] h-px min-w-8 flex-1 bg-black"/></span>}{data.date&&<span className="flex min-w-0 flex-1 items-end gap-2"><span className="shrink-0">Date:</span><span className="pupil-detail-line mb-[3px] h-px min-w-8 flex-1 bg-black"/></span>}</div>:null}</div><div className={`generic-grid grid flex-1 ${CONFIG.activityId==="barlines"||CONFIG.activityId==="timesig"||CONFIG.activityId==="triplets"||CONFIG.activityId==="rests"?"grid-cols-1":"grid-cols-2"} ${data.gridlines?"with-gridlines":""}`}>{items.map((q,i)=>{const example=!answers&&pageIndex===0&&i<examples.length; const n=answers?pageIndex*perPage+i+1:rhythmSums?(pageIndex===0?i-examples.length+1:perPage+(pageIndex-1)*perPage+i+1):pageIndex*perPage+i-examples.length+1; return <Question key={q.id} question={q} number={n} example={example} answers={answers} numbers={data.numbers} marks={data.marks} firstPage={!answers&&pageIndex===0} continuationPage={pageIndex>0}/>})}</div>{!answers&&data.marks&&pageIndex===pages.length-1?<div className="mt-3 flex justify-end gap-3 text-sm font-bold"><span className="h-8 leading-8">Total marks</span><span className="total-marks-box relative h-8 w-24 border border-black"><span className="total-marks-value absolute left-2 right-2 text-right leading-none">/ {data.questions.length}</span></span></div>:null}<footer className="mt-2 flex justify-between text-[10px] text-stone-400"><span>The Music Literacy Hub</span><span>Page {offset+pageIndex+1} of {total}</span></footer></article>);
  }
  ...
  function PracticePaperPage({ paper, data, paperIndex, answers=false, content="combined", pageNumber, totalPages }) {
    ...
    return <article className={`worksheet-page generic-page practice-paper-page ${paper.barCount>8?"practice-paper-page-sixteen":""} ${content==="score"?"practice-paper-score-only-page":""} ${content==="questions"?"practice-paper-questions-only-page":""}`} key={`${answers?"answers":"questions"}-${content}-${paper.id}-${paperIndex}`}>
      <div className="worksheet-header-card mb-2 rounded-xl border border-black px-4 py-3">
        <div className="flex items-center gap-3"><img src={DEF.icon} className="h-10 w-10 object-contain" alt=""/><div className="min-w-0"><h1 className="text-lg font-bold leading-tight">{answers ? `${data.title} - Answers` : data.title}</h1>{!answers&&<p className="text-xs text-stone-700">{data.instructions}</p>}</div></div>
  ...
  function PracticeWorksheetApp() {
    ...
    async function download(){
      setDownloading(true); const pages=[...document.querySelectorAll(".generic-page")];
      try { await document.fonts?.load?.("32px Bravura"); await document.fonts?.ready; pages.forEach(page=>page.classList.add("pdf-capture")); const {jsPDF}=window.jspdf; const pdf=new jsPDF({unit:"mm",format:"a4",orientation:"portrait"}); for(let index=0;index<pages.length;index+=1){if(index)pdf.addPage();const canvas=await html2canvas(pages[index],{scale:2.5,backgroundColor:"#fff",useCORS:true});pdf.addImage(canvas.toDataURL("image/png"),"PNG",0,0,210,297,undefined,"FAST");}pdf.save(`${window.MLH.safePdfTitle(title,DEF.title)}.pdf`); }
      catch(error){window.alert(error?.message||"The worksheet PDF could not be created. Please try again.");}
      finally{pages.forEach(page=>page.classList.remove("pdf-capture"));setDownloading(false);}
    }
