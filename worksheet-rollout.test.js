...
assert.match(generic, /children: \(\) => <div[\s\S]*>Time<\/span>[\s\S]*>0:00<\/span>[\s\S]*>Score<\/span>[\s\S]*>0\/\{concepts\.length\}<\/span>/, "Concept Recall worksheet mode should show inactive Time and Score cards rather than generic Score and Streak cards");
...
assert.match(generic, /used\[0\] <= used\[1\][\s\S]*startNewPage\(\)[\s\S]*const weightedGroups = conceptRecallGroups\(concepts\)[\s\S]*estimatedWeight/, "Concept Recall worksheet categories should be packed into the shortest column to reduce extra pages");
