...
test("interface includes the required screens, controls and protections", () => {
  ...
  assert.match(css, /\.millionaire-board\.is-creator\s*\{[^}]*height:\s*670px;[^}]*min-height:\s*670px;[^}]*overflow:\s*hidden;/s, "Opening Create should keep the same fixed game-window height as the main menu.");
  ...
  assert.match(css, /\.millionaire-creator-library-card\s*\{[^}]*display:\s*flex;[^}]*width:\s*min\(846px,\s*100%\);[^}]*min-height:\s*532px;[^}]*flex-direction:\s*column;[^}]*overflow:\s*auto;/s, "The Create library panel should match the rendered size of How to Play while retaining overflow for larger libraries.");
