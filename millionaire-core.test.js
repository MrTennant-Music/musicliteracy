...
test("interface includes the required screens, controls and protections", () => {
  ...
  assert.ok(script.includes('glyph={<img src="timer.svg"') && script.includes('className="h-5 w-5 object-contain"') && !script.includes('glyph={<img src="30timer.svg"'), "The Customise menu should use the shared Timer icon used elsewhere in the Hub.");
  ...
  assert.ok(script.includes('<span className="millionaire-opening-play-label">Play</span>'), "The opening button should read Play.");
  assert.ok(script.indexOf('>How to Play</span>') < script.indexOf('>Play</span>'), "How to Play should appear to the left of Play.");
  assert.ok(script.includes('onClick={startGame}><span className="millionaire-opening-play-label">Play</span>'), "Play should begin the game immediately.");
  ...
  assert.match(css, /\.millionaire-opening-screen,\s*\.millionaire-rules-screen\s*\{[^}]*height:\s*670px;[^}]*max-height:\s*670px;/s, "The main menu and How to Play screens should keep the same game-window height.");
  assert.ok(script.includes('className="millionaire-creator-global-toolbar millionaire-toolbar-wrap"') && script.includes('(screen !== "creator" || creatorEditing) && <div className="millionaire-toolbar-wrap">'), "The Create library should use one standard-position toolbar without an empty toolbar gap.");
  ...
  assert.match(script, /const DEFAULT_SETTINGS = \{[^}]*audioQuestions:\s*false,/, "Audio Questions should remain off until the listening bank is available.");
  ...
  assert.match(css, /\.millionaire-opening-play\s*\{[^}]*font-size:\s*30px;[^}]*line-height:\s*1;[^}]*text-transform:\s*none;/s, "Opening How to Play and Play should use title case and fill their button height.");
  ...
  assert.match(css, /\.millionaire-rules-card \.millionaire-rules-actions \.millionaire-secondary\s*\{[^}]*appearance:\s*none;[^}]*border:\s*1px solid #6eaee8;[^}]*background:\s*linear-gradient\(180deg,\s*#15498f 0%,\s*#08255b 100%\);[^}]*box-shadow:\s*none;/s, "The How to Play Back button should use the shared outlined blue gradient.");
  assert.match(css, /\.millionaire-opening-actions\s*\{[^}]*display:\s*flex;[^}]*justify-content:\s*center;/s, "How to Play and Play should sit together on one row.");
  ...
  assert.ok(!script.includes('label="Audio Questions"'), "Audio Questions should remain hidden until the listening bank is ready.");
  ...
  assert.ok(script.includes('const selectedQuestionCategories = () => ["literacy"];'), "Millionaire should use Music Literacy questions only while the other banks are coming soon.");
  ...
  assert.ok(!/\.millionaire-board\.is-creator\s*\{[^}]*height:|\.millionaire-board\.is-creator\s*\{[^}]*border-color:|\.millionaire-board\.is-creator\s*\{[^}]*background:/.test(css), "Create should inherit the standard board size, border and background rather than override them.");
  assert.match(css, /\.millionaire-creator-screen\s*\{[^}]*height:\s*670px;[^}]*min-height:\s*670px;[^}]*overflow-y:\s*auto;/s, "Long creator content should scroll inside the fixed game window.");
  assert.match(css, /\.millionaire-rules-card,\s*\.millionaire-creator-library-card\s*\{[^}]*width:\s*min\(780px,\s*100%\);[^}]*height:\s*572px;[^}]*min-height:\s*572px;[^}]*max-height:\s*572px;/s, "The Create and How to Play panels should use the same expanded fixed size.");
  assert.match(css, /\.millionaire-rules-actions\s*\{[^}]*justify-content:\s*center;[^}]*margin-top:\s*auto;/s, "The How to Play Back button should sit at the panel's padded bottom edge.");
  assert.match(css, /\.millionaire-rules-section\s*\{[^}]*border:\s*1px solid rgba\(125,211,252,\.6\);/s, "How to Play's inner panels should use the parent panel outline colour.");
  assert.match(css, /\.millionaire-creator-library-panel > h3\s*\{[^}]*color:\s*#f6c453;/s, "Create panel headings should use the same gold as How to Play headings.");
  assert.match(css, /\.millionaire-creator-instructions\s*\{[^}]*font-family:\s*Arial, sans-serif;[^}]*font-size:\s*15px;[^}]*font-weight:\s*400;[^}]*line-height:\s*1\.4;/s, "Create instructions should match the How to Play card text styling.");
  assert.match(css, /\.millionaire-creator-toolbar-actions \.millionaire-creator-toolbar-exit\s*\{[^}]*background:\s*#fff;[^}]*color:\s*#1c1917;/s, "The editor Exit button should use the neutral white toolbar style.");
  assert.match(css, /\.millionaire-creator-screen\.is-popover \.millionaire-set-card\s*\{[^}]*background:\s*rgba\(8,37,91,\.72\);/s, "Created-game cards should use the blue Create-panel styling.");
  assert.match(css, /\.millionaire-creator-screen\.is-popover \.millionaire-set-card h3,\s*\.millionaire-creator-screen\.is-popover \.millionaire-set-card h4\s*\{[^}]*color:\s*#fff;/s, "Created-game titles should be white on the blue cards.");
  assert.match(css, /\.millionaire-created-games-panel \.millionaire-set-card\s*\{[^}]*padding:\s*10px 14px;/s, "Created-game cards should use compact vertical padding.");
  assert.match(css, /\.millionaire-creator-screen\.is-popover\s*\{[^}]*background:\s*transparent;/s, "Create should inherit the standard board background without an extra highlight layer.");
  assert.match(css, /\.millionaire-creator-library-card\s*\{[^}]*display:\s*grid;[^}]*overflow:\s*auto;/s, "The Create library should retain scrolling for larger game libraries.");
  assert.match(css, /\.millionaire-creator-library-back-actions\s*\{[^}]*margin-top:\s*0;[^}]*padding-top:\s*14px;/s, "The Create Back button should use the same 14px gap as the purple button on the main menu.");
  assert.match(css, /\.millionaire-creator-screen\.is-popover \.millionaire-secondary\s*\{[^}]*appearance:\s*none;[^}]*border:\s*1px solid #6eaee8;[^}]*background:\s*linear-gradient\(180deg,\s*#15498f 0%,\s*#08255b 100%\);[^}]*box-shadow:\s*none;/s, "Secondary buttons in Create should use the shared outlined blue gradient.");
  assert.match(css, /\.millionaire-creator-screen\.is-popover \.millionaire-secondary\.is-danger\s*\{[^}]*border:\s*1px solid #fca5a5;[^}]*background:\s*#b91c1c;[^}]*box-shadow:\s*none;/s, "Delete should use the shared outlined red fill.");
  ...
  assert.ok(script.includes('const customiseUnavailable = openingZooming || screen === "game" || screen === "milestone" || screen === "creator";') && script.includes("<fieldset disabled={customiseUnavailable}"), "Customise should be unavailable during the opening transition, creator mode and while a game is active.");
  assert.ok(script.includes('millionaire-prize-completion-placeholder') && script.includes('aria-hidden="true">–</span>'), "The editor prize ladder should reserve a dash position for incomplete questions.");
  assert.ok(!script.includes('millionaire-prize-reward" src={reward.icon}'), "The prize ladder should not show diamond or medal icons.");
  assert.match(css, /\.millionaire-prize-completion-placeholder\s*\{[^}]*margin-left:\s*auto;[^}]*color:\s*#fff;/s, "Prize completion markers should be white and right-aligned without moving the prize amount.");
