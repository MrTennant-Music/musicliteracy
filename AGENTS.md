...
15a. Keep the automatic-marking notice on the left of the feedback footer row, with `Marking Instructions`, `Share` and `Try Again` together on its right. Stack the notice and actions only on narrow screens. Pressing Share first opens a centred `Share Feedback` prompt with an optional name field and unticked `Date` and `Time` checkboxes. When selected, place `Name: …`, the UK-formatted date and the 24-hour time beneath the `Results` heading in the PDF only. Use the browser’s native print-to-PDF rendering so Bravura notation, current answer boxes, colours and paper layouts match the HTML. Render the complete marked paper and restore the pupil’s previous feedback view after printing. Do not use screenshot conversion and do not show a separate Download action.
