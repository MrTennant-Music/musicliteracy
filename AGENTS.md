...
12. After submission, show a score card containing the current score, automatic-marking disclaimer and review actions, followed by a read-only All Questions rendering of the paper. Show the personal best inside the score card. Do not show a visible `Results` heading, topic breakdown or question-by-question accordion.
...
23. On feedback, place the score card beneath the in-container toolbar, followed by the automatic-marking disclaimer. Do not show a visible `Results` heading. Keep `20px` above and below the score card so its spacing from the toolbar matches its spacing from the disclaimer.
...
30. On each marked feedback paper, place the question’s awarded/available total at the bottom centre rather than beside the question heading. Give its container the same `65 × 44px` footprint and corner radius as the feedback navigation-arrow buttons. In single-question feedback, put the previous and next arrows in the same footer row on either side of the centred total; in All Questions feedback, show the total alone at the centre.
31. Do not use red or green background fills behind marked answers. Communicate correctness through the answer text, square-box outline, tick or notation symbol, and typed-answer line only: green for a correct response and red for an incorrect or unanswered response. Disable the pupil-input glimmer on marked ticks so it cannot override their fixed feedback colour. If a pupil selects an incorrect choice, add a green tick to each missed correct option; leave correct options unticked when the question was not answered. Keep the separate written feedback panels unchanged.
32. In the feedback summary, keep `Score`, `Percentage`, `Personal Best` and `Mode` in title case and use the earlier small, muted-grey stat-label styling. Keep their values in the established summary styling.
33. Keep inline answer-feedback boxes concise. Show only the result label, the correct answer when needed, a single short concept definition when the paper data provides one, and Question 8’s awarded concepts. Put the definition directly beneath `Correct answer`, source it from My Music Online’s A–Z Concepts and omit it rather than inventing wording when the source has no matching definition. Do not show part-mark fractions inside these boxes; the question total already appears in the bottom-centred score control. Do not restore generic explanation or rationale sentences.
